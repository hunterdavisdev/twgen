/**
 * twts core — pure, node-free. Safe to import from browser code (your token
 ...
 * utilities from Tailwind for free — twts only emits the `@theme` vars (plus the
...
const HEADER = "/* AUTO-GENERATED by twts — do not edit; regenerated from your tokens. */"
...
export function generateTheme(config: ThemesConfig): string {
	...
	if (themes.length === 0) throw new Error("twts: no themes defined")
	const defaults = themes.filter((t) => t.selector === ":root")
	if (defaults.length === 0)
		throw new Error('twts: no default theme — mark exactly one theme `default: true` (or give it selector ":root")')
	if (defaults.length > 1)
		throw new Error('twts: more than one default theme — only one theme may be `default: true` (selector ":root")')
	...
	for (const def of NAMESPACES) {
		...
		for (const t of themes) {
			for (const k of Object.keys(familyTokens(t, def))) {
				if (!known.has(k)) {
					throw new Error(`twts: ${def.ns} token "${k}" is set in theme "${t.name}" but missing from the default theme`)
				}
			}
		}
